...
export default function AnimationPlayground() {
  ...
  const worldZ = useTransform(progress, [0, 65, 80, 100], [0, 1100, 1100, 1750]);
  const leftDoorRotateY = useTransform(progress, [0, 65, 80, 100], [0, 0, 110, 110]);
  const rightDoorRotateY = useTransform(progress, [0, 65, 80, 100], [0, 0, -110, -110]);
  const ornamentsOpacity = useTransform(progress, [0, 45, 60, 100], [1, 1, 0, 0]);
  const carpetOpacity = useTransform(progress, [0, 80, 95, 100], [1, 1, 0, 0]);
  const doorFrameOpacity = useTransform(progress, [0, 80, 95, 100], [1, 1, 0, 0]);
  const stageOpacity = useTransform(progress, [0, 65, 80, 100], [0, 0, 1, 1]);
  ...
  const playAnimation = (startFrom = progress.get()) => {
    ...
    // Total duration is 7 seconds at 1x speed
    const duration = (7 * (remaining / 100)) / speed;
  ...
  const getStepLabel = (p) => {
    ...
    if (p < 60) return "Menuju Pintu Utama";
    if (p < 66) return "Tiba di Depan Pintu";
    if (p < 80) return "Membuka Pintu Utama";
    if (p < 95) return "Memasuki Panggung";
    return "Panggung Utama";
  };
  ...
  return (
    ...
    >
      ...
      >
        {/* Red Carpet */}
        <motion.div
          style={{ opacity: carpetOpacity, transformStyle: 'preserve-3d' }}
        ...
        >
          ...
          ></div>
          <div 
            className="absolute w-[200px] h-[2000px] bg-gradient-to-t from-transparent via-[#d4af37] to-transparent opacity-10" 
            style={{ 
              transform: 'translateY(530px) translateZ(-800px) rotateX(90deg)', 
              filter: 'blur(50px)' 
            }}
          ></div>
        </motion.div>

        {/* Ornamen Bali Gates, Wayang & Clouds */}
        <motion.div
      ...
      {/* Cinematic View Mode Toggle (Fades out when in cinematic mode unless hovered) */}
      <div 
        ...
        <div className="flex flex-col gap-4 bg-black/75 border border-[#d4af37]/40 rounded-2xl p-5 md:p-6 backdrop-blur-xl shadow-[0_10px_40px_rgba(0,0,0,0.8)]">
          ...
          <div className="flex flex-col gap-1 w-full">
            <div className="flex justify-between text-xs text-white/40 font-mono">
              <span>0% (Gerbang)</span>
              <span>65% (Tiba)</span>
              <span>80% (Terbuka)</span>
              <span>100% (Panggung)</span>
            </div>
            <div className="relative group w-full flex items-center h-6">
              ...
              <div className="absolute inset-x-0 h-1.5 bg-transparent pointer-events-none flex justify-between px-1">
                ...
                <div className="w-1.5 h-1.5 rounded-full bg-white/40 ml-[65%]" />
                <div className="w-1.5 h-1.5 rounded-full bg-white/40 ml-[15%]" />
                <div className="w-1.5 h-1.5 rounded-full bg-white/40" />
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-4 border-t border-white/10 pt-4">
            ...
            <div className="flex flex-wrap items-center gap-1.5">
              ...
              <button 
                onClick={() => jumpToStep(65)}
                className={`px-2.5 py-1 text-[11px] font-semibold rounded-md transition-all ${
                  currentProgress >= 60 && currentProgress < 66 
                    ? 'bg-[#d4af37]/20 text-[#d4af37] border border-[#d4af37]/40' 
                    : 'bg-white/5 text-white/60 hover:text-white border border-transparent'
                }`}
              >
              ...
              <button 
                onClick={() => jumpToStep(80)}
                className={`px-2.5 py-1 text-[11px] font-semibold rounded-md transition-all ${
                  currentProgress >= 78 && currentProgress < 85 
                    ? 'bg-[#d4af37]/20 text-[#d4af37] border border-[#d4af37]/40' 
                    : 'bg-white/5 text-white/60 hover:text-white border border-transparent'
                }`}
              >
